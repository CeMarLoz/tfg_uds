 ...
 * @author CesarMatias
 */
public class cRol implements ActionListener, DocumentListener, KeyListener {
    ...
    private void iniciar() {
        ...
        btnImprimir.setIcon(new ImageIcon(getClass().getResource("/img/print.png")));
        gridBagConstraints = new GridBagConstraints();
        gridBagConstraints.gridx = 2;
        gridBagConstraints.gridy = 0;
        gridBagConstraints.anchor = GridBagConstraints.LINE_END;
        this.vis.pDatos.add(btnImprimir, gridBagConstraints);
        ...
        cUtiles.habilitarCampos(this.vis.pDatos, false, new Object[]{this.txtCodigo, this.btnImprimir});
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == this.txtCodigo) {
            if (opAgregar == null && !this.txtCodigo.getText().isEmpty()) {
                ...
                // recuperando datos
                Iterator<eRol> itRol = this.mod.recuperaDatos(false).iterator();
        ...
        if (e.getSource() == this.vis.btnEliminar) {
            if (JOptionPane.showConfirmDialog(this.vis, "¿Seguro que desea Eliminar?", "CONFIRMAR!",
                ...
                String mensaje = this.mod.eliminarRol();
                if (mensaje == null) {
        ...
        if (e.getSource() == this.vis.btnGuardar) {
            ...
            if (opAgregar) { // si agregar fue seleccionado TRUE
                mensaje = this.mod.insertarRol();
            } else { // si agregar fue seleccionado FALSE
                mensaje = this.mod.actualizarRol();
            }
        ...
        if (e.getSource() == this.btnImprimir) {
            try {
                ...
                param.put("p_titulo", "Listado de Rol");

                JasperPrint jp = JasperFillManager.fillReport("src/reporte/rptDosCamDescri.jasper", param,
                        new JRBeanCollectionDataSource(this.mod.recuperaDatos(true)));
    ...
    public void keyTyped(KeyEvent e) {
        ...
        if (e.getSource() == txtDescripcion) {
            if (txtDescripcion.getText().length() > 64) {
                e.consume();
                Toolkit.getDefaultToolkit().beep();
            }
        }
