 ...
 * @author CesarMatias
 */
public class cCalle implements ActionListener, DocumentListener, KeyListener {
    ...
    // definicion boton
    private final JButton btnBuscarCalle;
     ...
     * @param vis
     */
    public cCalle(frmFormulario vis) {
        this.btnBuscarCalle = new JButton();
        ...
        this.btnBuscarCalle.addActionListener(this);
    ...
    private void iniciar() {
        ...
        btnBuscarCalle.setMnemonic('f');
        btnBuscarCalle.setIcon(new ImageIcon(getClass().getResource("/img/search.png")));
        gridBagConstraints = new GridBagConstraints();
        gridBagConstraints.gridx = 2;
        gridBagConstraints.gridy = 0;
        this.vis.pDatos.add(btnBuscarCalle, gridBagConstraints);
        ...
        cUtiles.habilitarCampos(this.vis.pDatos, false, new Object[]{this.btnBuscarCalle});
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == this.vis.btnAgregar) {
            opAgregar = true;
            // habilitacion de campos y botones a partir de las excepciones
            cUtiles.habilitarCampos(this.vis.pDatos, true, new Object[]{this.btnBuscarCalle});
        ...
        if (e.getSource() == this.vis.btnEliminar) {
            if (JOptionPane.showConfirmDialog(this.vis, "¿Seguro que desea Eliminar?", "CONFIRMAR!",
                ...
                // seteo de los campos
                this.mod.setCalCodigo(Integer.parseInt(this.txtCodigo.getText()));

                // llamada a la clase para insertar
                String mensaje = this.mod.eliminarCalle();
                if (mensaje == null) {
        ...
        if (e.getSource() == this.vis.btnGuardar) {
            ...
            // seteo de los campos
            this.mod.setCalCodigo(Integer.parseInt(this.txtCodigo.getText()));
            this.mod.setCalNombre(this.txtNombre.getText().trim());
            ...
            if (opAgregar) { // si agregar fue seleccionado TRUE
                mensaje = this.mod.insertarCalle();
            } else { // si agregar fue seleccionado FALSE
                mensaje = this.mod.actualizarCalle();
            }
        ...
        if (e.getSource() == this.btnBuscarCalle) {
            cBuscarCalle cBuscarCalle = new cBuscarCalle(mod, dlg);
            dlg.setVisible(true);

            Iterator<eCalle> itCalle = cBuscarCalle.devolverCalle().iterator();

            if (itCalle.hasNext()) {
                this.mod = itCalle.next();
                this.txtCodigo.setText(String.valueOf(this.mod.getCalCodigo()));
                this.txtNombre.setText(this.mod.getCalNombre());
            }
